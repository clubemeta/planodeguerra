...
switch ($action) {
    ...
    case 'diagnose':
        ...
        $all = loadBestFile();
        jsonResponse(['files'=>$files,'loaded'=>count($all),'free'=>min(50,count($all)),'soldado'=>min(1000,count($all)),'elite'=>count($all),'status'=>count($all)>0?'OK':'SEM QUESTÕES']);
        break;
    ...
    case 'stats':
        ...
        jsonResponse(['total'=>count($all),'disciplines'=>$discs,'free'=>min(50,count($all)),'soldado'=>min(1000,count($all)),'elite'=>count($all)]);
        break;
