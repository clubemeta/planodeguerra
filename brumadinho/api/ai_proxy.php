...
define('ANTHROPIC_API_KEY', 'COLE_SUA_CHAVE_AQUI'); // ← TROCAR
// ─────────────────────────────────────────────────────────
...
if (!$payload || !isset($payload['messages'])) {
    jsonError('Payload inválido.', 400);
}

// Tipo de uso
$isRedacao  = !empty($payload['redacao_correction']);
$isChat     = !empty($payload['chat_aluno']);
$isGerador  = !$isRedacao && !$isChat; // default: gerador de questões

// Limpar flags internas antes de enviar à Anthropic
unset($payload['redacao_correction'], $payload['chat_aluno']);
...
} elseif ($isRedacao || $isChat) {
    // Redação e Chat: Soldado e Elite
    if (!$user) {
        jsonError('Não autorizado. Faça login para usar este recurso.', 401);
    }
    if (!in_array($user['plan'] ?? '', ['soldado', 'elite']) && ($user['role'] ?? '') !== 'admin') {
        jsonError('Disponível nos planos Soldado e Elite. Faça upgrade para continuar!', 403);
    }
}
